...
Before editing, read the canonical `<topic>.prd.json` bundle plus its generated PRD and paired Canvas from `~/.claude/plugins/marketplaces/ctx-plugin/prds/<topic-slug>/`. Confirm:
...
The main session running this skill is the coordinator. It owns task selection, delegation, dependency judgment, integration, gate state, recovery, and user communication. Workers never re-orchestrate the project. If invoked inside a subagent or worker context, stop and emit the handoff packet from the ctx-prd artifact contract.

Use the routing table in [references/model-routing.md](references/model-routing.md):

- Sonnet subagents (medium/high effort): bounded research, repository reading, contract tracing, and evidence gathering; read-only.
- Implementer subagents: disjoint write ownership and exact gate outcomes.
- One fresh high-effort architect (Opus or stronger): only when complexity crosses the escalation threshold; otherwise omit it.
- `ctx-qa` or an equivalent independent verifier: gate verification on a running user-owned surface.
...
4. Ask the architect for a compact decision record only when escalation criteria apply.
5. Delegate implementation to one or more workers with disjoint files or worktrees, explicit interfaces, and targeted verification.
...
- Web UI: use `ctx-qa` (dev-browser CDP) against the existing user-owned app.
...
After two failed implementation approaches, require a fresh architecture review before another attempt. Do not weaken proof to make a gate pass.
