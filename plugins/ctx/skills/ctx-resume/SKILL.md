...
description: >
  ...
  Lists active plans from global storage and relaunches into the correct worktree.
...
List active plans from global storage and relaunch into the right worktree.

## Process

1. **Scan plans** — read all `.md` files in `~/.claude/plugins/marketplaces/ctx-plugin/plans/`
...
5. **Launch** — based on the plan's state:

| Plan state | Action |
|------------|--------|
| `worktree` path exists on disk | `bash ${CLAUDE_PLUGIN_ROOT}/scripts/worktree-open.sh '<worktree-path>'` |
| `worktree` gone but `branch` exists in git | Recreate worktree from branch via `worktree-create.sh`, then open |
| Neither (branch is null) | Offer to create a new worktree: ask for name, run `worktree-create.sh`, link the plan, then open |
...
Resuming: <topic>
  Worktree: <path>
  Branch:   <branch>
  Plan:     ~/.claude/plugins/marketplaces/ctx-plugin/plans/<topic>.md

New session opening. Run /ctx-execute to continue implementation.
```
