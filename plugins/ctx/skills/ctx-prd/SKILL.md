...
- The main session running this skill is the coordinator. It owns the interview, delegation, synthesis, final PRD judgment, and all bundle state. Workers never mutate the PRD, gate status, or scope.
- If this skill is invoked inside a subagent or worker context with no authority to coordinate, stop and emit the handoff packet from [references/artifact-contract.md](references/artifact-contract.md) for the user to open in a coordinator session.
- Delegate bounded read-only discovery to a Sonnet subagent (medium/high effort) when external evidence or current behavior can change the PRD.
- Route the review by risk (below); external review, when required, goes to one fresh high-effort reviewer subagent (Opus or stronger) with no drafting context.
...
4. For every gate, specify:
   ...
   - verifier: `ctx-qa`, automated test, log, screenshot, persisted state, or manual confirmation;
...
- Explore direction with the strongest available visual tools — the ctx factory (`ctx-brainstorm` prototypes browsed via `/ctx-factory`) or the Impeccable design skill. Generate genuinely independent directions and stop for the user's selection.
- Harden the selected direction (states, hierarchy, accessibility, responsive behavior) with Impeccable when available, or an equivalent structured UX pass, and record the findings.
...
Route the reviewer by risk: coordinator self-review with recorded evidence by default; one fresh external reviewer subagent (no drafting context, high effort) when the PRD touches an irreversible, security, payment, migration, or cross-system decision, or leaves architecture unresolved.
...
The canonical bundle lives on the local filesystem at `~/.claude/plugins/marketplaces/ctx-plugin/prds/<topic-slug>/`:
