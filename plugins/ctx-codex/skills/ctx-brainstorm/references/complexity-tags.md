...
Tag each unit of work in the spec. These tags flow into `ctx-plan` and `ctx-execute`:
...
Complexity tags set during brainstorming can be wrong — complexity is often discovered during implementation. `ctx-execute` should have an escape hatch to re-classify mid-flight.
