...
- **Designing a blocked phase**: Before brainstorming Phase N, state out loud which phase is the current "one battle" — the unshipped work that downstream phases depend on. If Phase N's design depends on UI or features from Phase N-1 (e.g., its locked ladder sells features the earlier phase will refactor, or its mockup embeds a component whose final shape isn't decided yet), stop and surface the sequencing question instead of brainstorming Phase N details. Symptoms: you catch yourself designing a mini preview of a component that doesn't exist in its final shape yet; your "locked features" list matches the blocker phase's scope; you keep asking tactical layout questions but can't point to an existing reference design for the underlying system. Momentum on a dependent phase is token burn on speculative UI — one well-timed sequencing question ("wait, is this phase blocked by X?") saves a 50-100k-token detour into components that will be rebuilt anyway.
...
- `./references/factory-guide.md` — visual factory CSS classes, loop, terminal-vs-browser guide
- `./factory/references/factory-frontend.md` — prototype design reference (read before generating)
- `./factory/` — server, frame template, launcher (read only when user accepts factory offer)
