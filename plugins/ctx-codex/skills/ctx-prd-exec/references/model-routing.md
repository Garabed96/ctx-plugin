# Model routing (Codex runtime)

"Sol" tiers name the main coding model at the stated reasoning effort.

| Role | Binding | Permissions | Returns |
|---|---|---|---|
| Coordinator | Main Codex session (high effort) | Coordinates; integrates; updates gate state | Decisions, task briefs, integrated result |
| Research/read | Sol Medium | Read-only | Findings, exact evidence, uncertainty |
| Implementation | Sol High | Scoped writes in owned files/worktree | Diff, tests, residual risk |
| Architect | Sol XHigh | Read-only unless explicitly reassigned | Decision record, interfaces, risks |
| Verifier | Computer use or independent test/observation | Read/test/observe | Pass/fail evidence per gate |
...
## Escalate to Sol XHigh when any apply
