...
description: >
  ...
  ctx-execute (which dispatches sequentially per-plan).
user-invocable: true
---

# ctx-parallel — Ad-Hoc Parallel Subagent Dispatch
...
## When This Skill vs ctx-execute

| Situation | Use |
|-----------|-----|
| You have a tagged plan from `ctx-plan` | `ctx-execute` |
| 2+ unrelated tasks, no plan needed | `ctx-parallel` |
| Multiple test failures across independent files | `ctx-parallel` |
| Sequential tasks that share state | `ctx-execute` |
...
- If this was part of a larger plan, return to `ctx-execute` for the next sequential task
