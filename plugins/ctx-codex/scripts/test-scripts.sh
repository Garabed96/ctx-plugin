#!/bin/bash
# test-scripts.sh — Integration tests for ctx-codex scripts.
...
if [[ ! -f .codex/skill-invocations.log ]]; then
...
# Main repo edit (we're in main checkout of ctx-codex test repo — should be allowed since it's a worktree-based flow)
