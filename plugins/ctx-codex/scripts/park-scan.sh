...
if [[ -d "$HOME/.codex/ctx-codex/plans" ]]; then
  while IFS= read -r f; do
    [[ -z "$f" ]] && continue
    ARTIFACTS="${ARTIFACTS}global-plan|${f}\n"
  done < <(find "$HOME/.codex/ctx-codex/plans" -name '*.md' -type f -mtime -1 2>/dev/null)
fi

# ── Skill invocation log ─────────────────────────────────
SKILL_LOG=""
LOG_PATH="$REPO_ROOT/.codex/skill-invocations.log"
if [[ -f "$LOG_PATH" ]]; then
