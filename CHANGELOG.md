...
- **First-class Oh My Pi runtime adapter** — adds the `ctx-omp` plugin with 24 curated CTX skills, three bounded task agents, native profile/repository path resolution, validated local workflow operations, lifecycle policy hooks, and final-HEAD-bound interactive approval for remote shipping. Factory V1 and the superseded brainstorm skills are intentionally excluded from OMP while remaining in the Claude and Codex packages; the shared parity manifest records these runtime differences.
...
- **Digestible plan/PRD layout** — codifies the deterministic lean-plan layout into `ctx-lean` (template + four layout rules: one named orienting question per mermaid diagram with ≤15 nodes, intuition-before-mechanics framing per MED/HIGH task cluster, decisions dated and owner-attributed with accepted cost, evidence pinned to a commit SHA) and into `ctx-prd` as authoring guidance for the free-text bundle fields plus a dated owner-decision placeholder in the Markdown template.

## 0.2.9.13 (2026-07-30)

- **CTX PRD + CTX PRD Exec** — adds mirrored `ctx-prd` and `ctx-prd-exec` skills for gated Demo PRDs. `ctx-prd` turns product intent into a canonical `<topic>.prd.json` bundle with generated Markdown and Obsidian Canvas projections: 3–5 ordered, evidence-gated demo gates, 1–5 real guardrails, a five-question review checklist routed by risk (coordinator self-review by default, one fresh external reviewer for irreversible/security/payment/migration/cross-system decisions), and a user-selection invariant for new visual direction with a documented waiver for existing-design-system changes. Storage is filesystem-canonical per runtime (`~/.claude/plugins/marketplaces/ctx-plugin/prds/`, `~/.codex/ctx-codex/prds/`); the Obsidian War Room sync via `obsidian-war-room` MCP is a required adapter — an outage marks the bundle `UNSYNCED`, which blocks `READY` but never blocks drafting or revokes granted execution authority. `ctx-prd-exec` executes one gate at a time with just-in-time slices, disjoint worker ownership, verifier-didn't-implement separation, cross-gate interface records, and no-fake-pass rules (`WAITING_FOR_MANUAL_CONFIRMATION` instead of claiming). A bundled renderer validates the state machine — including a representable `BLOCKED` bundle state with `blockedReason` and user-`acceptedException` gate skips — and a self-test covers pass and rejection paths. Roles live in shared prose; model names bind per mirror (Sonnet/Opus subagents on Claude, Sol effort tiers on Codex).
